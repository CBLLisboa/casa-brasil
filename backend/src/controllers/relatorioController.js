...
async function relatorioFamiDesagregacao(req, res, next) {
  try {
    ...
    let atendimentosPorNpt = [];
    try {
      const atendNpt = await db.query(
        `SELECT
          p.id AS pessoa_id,
          p.nome_completo AS nome,
          a.data_atendimento::date AS data,
          COUNT(*)::int AS n_atendimentos
        FROM atendimentos a
        JOIN pessoas p ON p.id = a.beneficiario_id
        ${whereClause}
        GROUP BY p.id, p.nome_completo, a.data_atendimento::date
        ORDER BY p.nome_completo, a.data_atendimento::date`,
        valores
      );
      atendimentosPorNpt = atendNpt.rows || [];
    } catch (err) {
      console.error("Erro ao carregar atendimentos por NPT:", err);
    }
    ...
    return res.json({
      ...
      atendimentos_por_npt: atendimentosPorNpt
    });
  } catch (error) {
    return next(error);
  }
