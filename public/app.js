...
if (criarPessoaTopoBtn) {
  criarPessoaTopoBtn.addEventListener("click", () => {
    if (topoBusca) topoBusca.scrollIntoView({ behavior: "smooth" });
    iniciarNovoRegistro();
  });
}
...
async function buscarParentes() {
  ...
  pessoas.forEach((pessoa) => {
    ...
    option.textContent = `${pessoa.nome_completo} (${pessoa.numero_documento || "sem doc"})`;
  ...
  if (pessoas.length === 0) {
    ...
    mostrarMensagem(`Encontradas ${pessoas.length} pessoas.`, true);
    if (pessoas.length === 1) {
      parenteSelect.value = pessoas[0].id;
      campoParenteId.value = pessoas[0].id;
...
buscarParenteBtn.addEventListener("click", () => {
...
async function carregarAtendimentoParaEdicao(id) {
  ...
  try {
    ...
    if (submitBtn) submitBtn.textContent = "Guardar correção";
    if (avaliouSelect && servicoUtilWrapper) {
