...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    try {
      ...
      mostrarMensagem("Resposta inválida do servidor. Aguarde 1 min (servidor a iniciar) e tente novamente.");
      return;
    }
    if (!resposta.ok) {
    ...
    if (msg.includes("Failed to fetch") || msg.includes("NetworkError")) {
      mostrarMensagem("Servidor indisponível. Aguarde ~1 min (plano gratuito) e tente novamente.");
    } else {
