...
async function gerarCodigoFuncionario() {
  ...
  const sql = db.isMySQL
    ? `SELECT COALESCE(MAX(CAST(REGEXP_REPLACE(COALESCE(codigo,''), '[^0-9]', '') AS UNSIGNED)), 0) AS max FROM funcionarios`
    : `SELECT COALESCE(MAX(NULLIF(regexp_replace(codigo, '\\D', '', 'g'), '')::int), 0) AS max FROM funcionarios`;
